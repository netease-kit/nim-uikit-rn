 ...
 * 消息转发组件
 */
const MessageForward: React.FC<MessageForwardProps> = observer(
  ({ visible, msgIdClient, onClose }) => {
    ...
    const [forwardConversationType, setForwardConversationType] =
      useState<V2NIMConst.V2NIMConversationType>(
    ...
    const handleForwardConfirm = (forwardComment: string) => {
      ...
      const methodName =
        forwardConversationType === V2NIMConst.V2NIMConversationType.V2NIM_CONVERSATION_TYPE_P2P
          ? 'p2pConversationId'
          : 'teamConversationId'
        ...
        >
          <div className="forward-container-wrapper">
            <div className="tab-container">
              <div
                className={`tab-item ${currentTab === 'friend' ? 'active' : ''}`}
                onClick={() => switchTab('friend')}
              >
                {t('sendToFriend')}
              </div>
              <div
                className={`tab-item ${currentTab === 'team' ? 'active' : ''}`}
