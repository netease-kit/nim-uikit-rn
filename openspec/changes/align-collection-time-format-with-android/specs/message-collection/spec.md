...
#### Scenario: Rendering today's collection timestamp

- **GIVEN** a collection entry create time is on the same local calendar day as the current time
- **WHEN** the collection card renders its footer timestamp
- **THEN** the timestamp MUST display as `HH:mm` using 24-hour time

#### Scenario: Rendering same-year collection timestamp

- **GIVEN** a collection entry create time is in the same local calendar year as the current time but not today
- **WHEN** the collection card renders its footer timestamp
- **THEN** the timestamp MUST display as `MM月dd日 HH:mm` using 24-hour time

#### Scenario: Rendering cross-year collection timestamp

- **GIVEN** a collection entry create time is not in the same local calendar year as the current time
- **WHEN** the collection card renders its footer timestamp
