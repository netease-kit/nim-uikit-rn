...
import {
  ...
  AT_ALL_ACCOUNT,
  ALLOW_AT
} from '@/NEUIKit/common/utils/constants'
...
const MessageInput: React.FC<MessageInputProps> = observer(
  ({ conversationType, to, replyMsgsMap = {} }) => {
    ...
    const [selectedAtMembers, setSelectedAtMembers] = useState<
      { accountId: string; appellation: string }[]
    >([])
    // 修改为使用正确的类型
    ...
    const teamMute =
      team?.chatBannedMode || V2NIMConst.V2NIMTeamChatBannedMode.V2NIM_TEAM_CHAT_BANNED_MODE_UNBAN

    const [isTeamOwner, setIsTeamOwner] = useState(false)
    const [isTeamManager, setIsTeamManager] = useState(false)
    const isTeamMute = useMemo(() => {
      if (teamMute === V2NIMConst.V2NIMTeamChatBannedMode.V2NIM_TEAM_CHAT_BANNED_MODE_UNBAN)
        return false
    ...
    const handleMentionSelect = (member) => {
      ...
      setSelectedAtMembers([
        ...selectedAtMembers.filter((item) => item.accountId !== member.accountId),
        member
      ])
        ...
        {/** 艾特消息弹出层 */}
        {
          ...
          >
            <MentionChooseList
              ...
              onMemberClick={handleMentionSelect}
            ></MentionChooseList>
          </BottomPopup>
        }
      </div>
