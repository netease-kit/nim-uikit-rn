...
#### Scenario: Dispatching multiple selected media items

- **WHEN** the user selects multiple images from the chat media picker
- **THEN** RN MUST prepare and dispatch the selected image messages without waiting for the previous selected image upload or send callback to finish first
- **AND** if the picker returns videos, RN MUST apply the same non-serial dispatch rule to the selected video messages
- **AND** one selected asset failing preparation or send MUST NOT prevent the other valid selected assets from entering their normal send flow
