...
- **AND** the marked-message background MUST fill the full message row width, including the checkbox lane
...
- **WHEN** the forwarding confirmation dialog appears
- **THEN** the dialog MUST show at most six selected target avatars
- **AND** a seventh selected target MUST NOT render as an avatar or overflow counter
