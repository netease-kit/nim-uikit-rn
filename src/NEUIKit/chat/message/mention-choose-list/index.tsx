...
const MentionChooseList: React.FC<MentionChooseListProps> = ({
  ...
  const teamMembersWithoutSelf = useMemo(() => {
    return teamMembers.filter((item) => item.accountId !== store?.userStore.myUserInfo.accountId)
      ...
      <div className="member-list-content">
        ...
        {allowAtAll && (
          ...
          >
            ...
            <span className="member-name">{t('teamAll')}</span>
          </div>
        )}
        ...
        {teamMembersWithoutSelf.map((member) => (
          ...
          >
            ...
            <div className="member-name">
              <Appellation account={member.accountId} teamId={member.teamId} />
            </div>
            {member.memberRole === V2NIMConst.V2NIMTeamMemberRole.V2NIM_TEAM_MEMBER_ROLE_OWNER && (
              <div className="owner">{t('teamOwner')}</div>
            )}
            {member.memberRole ===
              V2NIMConst.V2NIMTeamMemberRole.V2NIM_TEAM_MEMBER_ROLE_MANAGER && (
              <div className="manager">{t('teamManager')}</div>
            )}
          </div>
        ))}
      </div>
