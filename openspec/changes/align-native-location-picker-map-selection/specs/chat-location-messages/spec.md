...
#### Scenario: Pick location by moving map center

- **GIVEN** the user is on the native location picker screen
- **WHEN** the user drags the map or re-centers the map to a new position while no keyword search is active
- **THEN** the app SHALL treat the map center as the current selection anchor
- **AND** the nearby POI list SHALL refresh around the new center
- **AND** the default selected result SHALL track the new center instead of reverting to a stale current-location row
