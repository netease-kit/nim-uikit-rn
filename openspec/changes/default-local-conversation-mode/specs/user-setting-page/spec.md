...
#### Scenario: Fresh install defaults to local conversation mode

- **WHEN** the app starts without any persisted cloud-conversation preference
- **THEN** the cloud-conversation toggle default value MUST be off
- **AND** the app MUST initialize NIM in local conversation mode by default
