# chat-location-messages Specification

## Purpose

TBD - created by archiving change support-location-messages. Update Purpose after archive.

## Requirements

### Requirement: Chat Location Message Send Flow

The chat detail screen SHALL NOT expose a composer action for choosing and sending a location message.

#### Scenario: Location send entry is removed

- **WHEN** the user opens the chat detail more panel
- **THEN** RN MUST NOT show a location send action
- **AND** RN MUST NOT open the location picker from the chat detail composer
